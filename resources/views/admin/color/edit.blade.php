...
$pageMenu = 'color';
if ($saveMode == 'add') {
    $pageSubTitle = trans('admin.color.form.add.sub_title');
    $url = route('admin_color_store');
} else {
    $pageSubTitle = trans('admin.color.form.edit.sub_title', ['id' => $color->id]);
    $url = route('admin_color_update', $color->id);
}
...
<form id="edit-form" class="form-horizontal" method="post" action="{{$url}}">
    ...
    <div class="box-footer">
        ...
        <a href="{{route('admin_color_table')}}" class="nav-btn nav-btn-cancel btn btn-default">{{trans('admin.base.label.cancel')}}</a>
    </div>
</form>
@stop