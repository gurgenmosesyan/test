...
$pageMenu = 'engine';
if ($saveMode == 'add') {
    $pageSubTitle = trans('admin.engine.form.add.sub_title');
    $url = route('admin_engine_store');
} else {
    $pageSubTitle = trans('admin.engine.form.edit.sub_title', ['id' => $engine->id]);
    $url = route('admin_engine_update', $engine->id);
}
$mls = $engine->ml->keyBy('lng_id');
?>
@extends('core.layout')
@section('content')
<form id="edit-form" class="form-horizontal" method="post" action="{{$url}}">
    ...
    <div class="box-footer">
        ...
        <a href="{{route('admin_engine_table')}}" class="nav-btn nav-btn-cancel btn btn-default">{{trans('admin.base.label.cancel')}}</a>
    </div>
</form>
@stop